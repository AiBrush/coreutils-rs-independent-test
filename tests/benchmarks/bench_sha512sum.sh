...
GNU_TOOL="sha512sum"
F_TOOL="fsha512sum"
U_TOOL="${UUTILS_DIR:+$UUTILS_DIR/sha512sum}"

run_sha512sum_benchmarks() {
    check_hyperfine
    ensure_test_data

    if ! check_tool_exists "$F_TOOL"; then
        echo '{"tool":"sha512sum","status":"NOT_IMPLEMENTED"}' > "$RESULTS_DIR/sha512sum_benchmark.json"
        return 0
    fi

    init_benchmark "sha512sum"
