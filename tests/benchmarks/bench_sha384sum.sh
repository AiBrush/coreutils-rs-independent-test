...
GNU_TOOL="sha384sum"
F_TOOL="fsha384sum"
U_TOOL="${UUTILS_DIR:+$UUTILS_DIR/sha384sum}"

run_sha384sum_benchmarks() {
    check_hyperfine
    ensure_test_data

    if ! check_tool_exists "$F_TOOL"; then
        echo '{"tool":"sha384sum","status":"NOT_IMPLEMENTED"}' > "$RESULTS_DIR/sha384sum_benchmark.json"
        return 0
    fi

    init_benchmark "sha384sum"
