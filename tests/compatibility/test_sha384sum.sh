#!/usr/bin/env bash
# Compatibility tests for fsha384sum vs GNU sha384sum
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../common.sh"

GNU_TOOL=$(resolve_gnu_tool "sha384sum")
F_TOOL="fsha384sum"
...
run_sha384sum_tests() {
    ...
    if ! check_tool_exists "$GNU_TOOL"; then
        echo "GNU sha384sum not found, cannot compare" >&2
        return 1
    fi
    ...
    run_test "check mode" \
        "$GNU_TOOL '$TEST_DATA_DIR/checksum_abc.txt' > /tmp/sha384_gnu_$$ && $GNU_TOOL -c /tmp/sha384_gnu_$$ && rm -f /tmp/sha384_gnu_$$" \
    ...
    run_gnu_upstream_tests "sha384sum"
