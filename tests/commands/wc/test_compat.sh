...
run_wc_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: --total Option ==="

    # From wc-total.sh: --total=never suppresses total line
    local wc_tmp1=$(make_temp "$(printf '2\n')")
    register_temp "$wc_tmp1"
    local wc_tmp2=$(make_temp "$(printf '2 words\n')")
    register_temp "$wc_tmp2"

    run_test "--total=never two files" \
        "$GNU_TOOL --total=never '$wc_tmp1' '$wc_tmp2'" \
        "$F_TOOL --total=never '$wc_tmp1' '$wc_tmp2'"

    run_test "--total=only two files" \
        "$GNU_TOOL --total=only '$wc_tmp1' '$wc_tmp2'" \
        "$F_TOOL --total=only '$wc_tmp1' '$wc_tmp2'"

    run_test "--total=always single file" \
        "$GNU_TOOL --total=always '$wc_tmp1'" \
        "$F_TOOL --total=always '$wc_tmp1'"

    # --total with no value should error
    run_exit_code_test "--total invalid (error)" \
        "$GNU_TOOL --total '$wc_tmp1' '$wc_tmp2' 2>&1" \
        "$F_TOOL --total '$wc_tmp1' '$wc_tmp2' 2>&1"

    echo ""
    echo "=== GNU Upstream: NBSP Word Separation ==="

    # From wc-nbsp.sh: non-breaking space treated as word separator in UTF-8
    # Test basic NBSP handling (U+00A0)
    local wc_nbsp_tmp=$(make_temp "$(printf '=\xc2\xa0=')")
    register_temp "$wc_nbsp_tmp"
    run_test "NBSP as word separator (UTF-8)" \
        "LC_ALL=en_US.UTF-8 $GNU_TOOL -w '$wc_nbsp_tmp'" \
        "LC_ALL=en_US.UTF-8 $F_TOOL -w '$wc_nbsp_tmp'"

    # Standard space for comparison
    local wc_space_tmp=$(make_temp "$(printf '= =')")
    register_temp "$wc_space_tmp"
    run_test "regular space as word separator" \
