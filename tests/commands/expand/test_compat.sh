...
GNU_TOOL=$(resolve_gnu_tool "expand")
F_TOOL="fexpand"
...
run_expand_tests() {
    ...
    # === GNU Upstream: Bounded Memory / Large Input ===
    echo ""
    echo "=== GNU Upstream: Bounded Memory / Large Input ==="

    run_test "large repeated tab input" \
        "printf '%0.s\t' {1..1000} | $GNU_TOOL | wc -c" \
        "printf '%0.s\t' {1..1000} | $F_TOOL | wc -c"

    run_test "long line with tabs" \
