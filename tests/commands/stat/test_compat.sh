...
GNU_TOOL=$(resolve_gnu_tool "stat")
F_TOOL="fstat"
...
run_stat_tests() {
    ...
    # === Section 7: GNU Upstream Tests ===
    echo ""
    echo "=== GNU Upstream: Format String Edge Cases ==="

    # From stat-fmt.sh: format string ending with % (literal percent)
    run_stdout_test "format ending with %" \
        "$GNU_TOOL --format='%' '$test_dir/regular_file'" \
        "$F_TOOL --format='%' '$test_dir/regular_file'"

    run_stdout_test "format with multiple trailing %" \
        "$GNU_TOOL --format='%%%' '$test_dir/regular_file'" \
        "$F_TOOL --format='%%%' '$test_dir/regular_file'"

    # From stat-fmt.sh: %N quoting style
    run_stdout_test "%N file name quoting" \
        "$GNU_TOOL -c '%N' '$test_dir/regular_file'" \
        "$F_TOOL -c '%N' '$test_dir/regular_file'"

    # From stat-fmt.sh: %H and %L device modifiers
    run_stdout_test "%Hd,%Ld device major,minor" \
        "$GNU_TOOL -c '%Hd,%Ld' '$test_dir/regular_file'" \
        "$F_TOOL -c '%Hd,%Ld' '$test_dir/regular_file'"

    echo ""
    echo "=== GNU Upstream: Hyphen Filename ==="

    # From stat-hyphen.sh: stat - reads stdin as file named -
    run_stdout_test "stat --format=%n - (hyphen filename)" \
        "$GNU_TOOL --format=%n - < '$test_dir/regular_file'" \
        "$F_TOOL --format=%n - < '$test_dir/regular_file'"

    # From stat-hyphen.sh: stat -f - should fail
    run_exit_code_test "stat -f - (error)" \
        "$GNU_TOOL -f - < '$test_dir/regular_file' 2>&1" \
        "$F_TOOL -f - < '$test_dir/regular_file' 2>&1"

    echo ""
    echo "=== GNU Upstream: Trailing Slash Handling ==="

    # From stat-slash.sh: trailing slash on symlink to file should error
    ln -sf "$test_dir/regular_file" "$test_dir/link_to_file" 2>/dev/null || true
    ln -sf "$test_dir/a_directory" "$test_dir/link_to_dir" 2>/dev/null || true

    run_stdout_test "%n on symlink (no slash)" \
        "$GNU_TOOL --format=%n '$test_dir/link_to_file'" \
        "$F_TOOL --format=%n '$test_dir/link_to_file'"

    run_exit_code_test "symlink/ to file (error)" \
        "$GNU_TOOL --format=%n '$test_dir/link_to_file/' 2>&1" \
        "$F_TOOL --format=%n '$test_dir/link_to_file/' 2>&1"

    run_stdout_test "%F on symlink to dir (no -L)" \
        "$GNU_TOOL --format=%F '$test_dir/link_to_dir'" \
        "$F_TOOL --format=%F '$test_dir/link_to_dir'"

    run_stdout_test "%F on symlink/ to dir (deref via slash)" \
        "$GNU_TOOL --format=%F '$test_dir/link_to_dir/'" \
        "$F_TOOL --format=%F '$test_dir/link_to_dir/'"
