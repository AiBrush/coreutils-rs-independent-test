...
GNU_TOOL=$(resolve_gnu_tool "sha224sum")
F_TOOL="fsha224sum"
...
run_sha224sum_tests() {
    ...
    local checkfile
    checkfile=$(mktemp /tmp/fcoreutils_sha224check_XXXXXX)
    ...
    # Wrong hash
    local bad_checkfile
    bad_checkfile=$(mktemp /tmp/fcoreutils_sha224bad_XXXXXX)
    register_temp "$bad_checkfile"
    echo "00000000000000000000000000000000000000000000000000000000  $TEST_DATA_DIR/checksum_abc.txt" > "$bad_checkfile"
    ...
    missing_checkfile=$(mktemp /tmp/fcoreutils_sha224missing_XXXXXX)
    register_temp "$missing_checkfile"
    echo "d14a028c2a3a2bc9476102bb288234c415a2b01f828ea62ac5b3e42f  /tmp/nonexistent_file_$$" > "$missing_checkfile"
    ...
    malformed_checkfile=$(mktemp /tmp/fcoreutils_sha224malformed_XXXXXX)
    ...
    local test_dir
    test_dir=$(mktemp -d /tmp/fcoreutils_sha224sum_XXXXXX)
