...
GNU_TOOL=$(resolve_gnu_tool "nohup")
F_TOOL="fnohup"
...
run_nohup_tests() {
    ...
    # === GNU Upstream: Exit Code for Missing/Non-Executable Commands ===
    echo ""
    echo "=== GNU Upstream: Exit Code for Missing/Non-Executable Commands ==="

    run_test "nonexistent command exits 127" \
        "$GNU_TOOL no-such-command 2>/dev/null; echo \$?" \
        "$F_TOOL no-such-command 2>/dev/null; echo \$?"

    local NX_FILE="$WORK_DIR/not_executable"
    touch "$NX_FILE"
    chmod 0 "$NX_FILE"

    run_test "non-executable file exits 126" \
        "$GNU_TOOL '$NX_FILE' 2>/dev/null; echo \$?" \
        "$F_TOOL '$NX_FILE' 2>/dev/null; echo \$?"

    # === GNU Upstream: No Arguments Exit Code ===
    echo ""
    echo "=== GNU Upstream: No Arguments Exit Code ==="

    run_test "no args exits 125" \
        "$GNU_TOOL 2>/dev/null; echo \$?" \
        "$F_TOOL 2>/dev/null; echo \$?"

    run_test "POSIXLY_CORRECT no args exits 127" \
        "POSIXLY_CORRECT=1 $GNU_TOOL 2>/dev/null; echo \$?" \
        "POSIXLY_CORRECT=1 $F_TOOL 2>/dev/null; echo \$?"

    # === GNU Upstream: Stdout Redirection ===
    echo ""
    echo "=== GNU Upstream: Stdout Redirection ==="

    run_test "stdout redirect suppresses nohup.out" \
        "cd '$WORK_DIR' && rm -f nohup.out && $GNU_TOOL sh -c 'echo stdout' > /dev/null 2>&1; test ! -f nohup.out && echo 'NO_FILE' || echo 'FILE_EXISTS'" \
        "cd '$WORK_DIR' && rm -f nohup.out && $F_TOOL sh -c 'echo stdout' > /dev/null 2>&1; test ! -f nohup.out && echo 'NO_FILE' || echo 'FILE_EXISTS'"
