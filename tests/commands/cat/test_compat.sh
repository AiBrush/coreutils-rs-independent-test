...
run_cat_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Line Ending Display (-E) ==="

    # From cat-E.sh: \r followed by \n displayed as ^M$
    local cat_e_tmp=$(make_temp "$(printf 'a\rb\r\nc\n\r\nd\r')")
    register_temp "$cat_e_tmp"
    run_test "-E CR/LF display" \
        "$GNU_TOOL -E '$cat_e_tmp'" \
        "$F_TOOL -E '$cat_e_tmp'"

    # From cat-E.sh: \r\n spanning two files
    local cat_e_f1=$(make_temp "$(printf '1\r')")
    register_temp "$cat_e_f1"
    local cat_e_f2=$(make_temp "$(printf '\n2\r\n')")
    register_temp "$cat_e_f2"
    run_test "-E CR/LF across files" \
        "$GNU_TOOL -E '$cat_e_f1' '$cat_e_f2'" \
        "$F_TOOL -E '$cat_e_f1' '$cat_e_f2'"

    # From cat-E.sh: \r at end of buffer followed by non-\n
    local cat_e_f3=$(make_temp "$(printf '1\r')")
    register_temp "$cat_e_f3"
    local cat_e_f4=$(make_temp "$(printf '2\r\n')")
    register_temp "$cat_e_f4"
    run_test "-E CR at buffer boundary" \
        "$GNU_TOOL -E '$cat_e_f3' '$cat_e_f4'" \
        "$F_TOOL -E '$cat_e_f3' '$cat_e_f4'"

    echo ""
    echo "=== GNU Upstream: Self-Reading Detection ==="

    # From cat-self.sh: cat input >> same file should fail
    local cat_self_dir
    cat_self_dir=$(mktemp -d /tmp/fcoreutils_catself_XXXXXX)
    register_temp "$cat_self_dir"
    echo x > "$cat_self_dir/out_gnu"
    echo x > "$cat_self_dir/out_f"
    run_exit_code_test "cat file >> same file (error)" \
        "$GNU_TOOL '$cat_self_dir/out_gnu' >> '$cat_self_dir/out_gnu' 2>&1" \
        "$F_TOOL '$cat_self_dir/out_f' >> '$cat_self_dir/out_f' 2>&1"

    # From cat-self.sh: POSIX example - cat doc doc.end > doc
    echo x > "$cat_self_dir/doc_gnu"
    echo y > "$cat_self_dir/doc_end_gnu"
    echo x > "$cat_self_dir/doc_f"
    echo y > "$cat_self_dir/doc_end_f"
    run_exit_code_test "cat file1 file2 > file1 (POSIX)" \
        "($GNU_TOOL '$cat_self_dir/doc_gnu' '$cat_self_dir/doc_end_gnu' > '$cat_self_dir/doc_gnu' 2>&1; echo \$?)" \
        "($F_TOOL '$cat_self_dir/doc_f' '$cat_self_dir/doc_end_f' > '$cat_self_dir/doc_f' 2>&1; echo \$?)"
