...
GNU_TOOL=$(resolve_gnu_tool "unexpand")
F_TOOL="funexpand"
...
run_unexpand_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Bounded Memory ==="

    # GNU bounded-memory.sh: test that unexpand uses bounded memory on /dev/zero
    # Skipped: requires get_min_ulimit_v_, ulimit -v, timing-dependent, and /dev/zero
    skip_test "bounded memory on infinite input" "Requires ulimit -v and timing-dependent infrastructure"
