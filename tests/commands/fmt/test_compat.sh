...
EOF
    ...
    echo ""
    echo "=== GNU Upstream: Goal Width (-g) ==="

    # From goal-option.sh: fmt -g 60 -w 72 on paragraph text
    cat > "$test_dir/goal_input.txt" <<'GOALTXT'
...
GOALTXT
    ...
    run_stdout_test "-g 60 -w 72 paragraph" \
        "$GNU_TOOL -g 60 -w 72 '$test_dir/goal_input.txt'" \
        "$F_TOOL -g 60 -w 72 '$test_dir/goal_input.txt'"

    echo ""
    echo "=== GNU Upstream: Long Line Split ==="

    # From long-line.sh: fmt -s on very long line (1015 ' y' tokens)
    printf '%2030s\n' ' ' | sed 's/../ y/g' > "$test_dir/longline_gnu.txt"
    run_stdout_test "-s very long line (1015 tokens)" \
        "$GNU_TOOL -s '$test_dir/longline_gnu.txt'" \
        "$F_TOOL -s '$test_dir/longline_gnu.txt'"

    echo ""
    echo "=== GNU Upstream: Width Correctness ==="

    # From width.sh: ensure width is max display width (not including \n)
    local fmt_w_tmp1=$(make_temp "$(printf 'aa bb cc dd ee')")
    register_temp "$fmt_w_tmp1"
    run_stdout_test "-w 8 exact width" \
        "$GNU_TOOL -w 8 '$fmt_w_tmp1'" \
        "$F_TOOL -w 8 '$fmt_w_tmp1'"

    local fmt_w_tmp2=$(make_temp "$(printf 'aa bb cc dd ee')")
    register_temp "$fmt_w_tmp2"
    run_stdout_test "-w 7 narrow width" \
        "$GNU_TOOL -w 7 '$fmt_w_tmp2'" \
        "$F_TOOL -w 7 '$fmt_w_tmp2'"
