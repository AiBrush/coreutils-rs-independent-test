...
run_od_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: -N Limit Reads ==="

    # From od-N.sh: sequential reads with -N from same stdin
    local od_n_tmp=$(make_temp "abcdefg
")
    register_temp "$od_n_tmp"
    run_stdout_test "-N3 -c sequential reads" \
        "(($GNU_TOOL -An -N3 -c; $GNU_TOOL -An -N3 -c) < '$od_n_tmp')" \
        "(($F_TOOL -An -N3 -c; $F_TOOL -An -N3 -c) < '$od_n_tmp')"

    echo ""
    echo "=== GNU Upstream: Endian Control ==="

    # From od-endian.sh: verify --endian swaps byte order
    local od_endian_tmp=$(make_temp "$(printf '0123456789abcdef')")
    register_temp "$od_endian_tmp"
    run_stdout_test "--endian=little -t x2" \
        "$GNU_TOOL -An -t x2 --endian=little '$od_endian_tmp'" \
        "$F_TOOL -An -t x2 --endian=little '$od_endian_tmp'"

    run_stdout_test "--endian=big -t x2" \
        "$GNU_TOOL -An -t x2 --endian=big '$od_endian_tmp'" \
        "$F_TOOL -An -t x2 --endian=big '$od_endian_tmp'"

    run_stdout_test "--endian=little -t x4" \
        "$GNU_TOOL -An -t x4 --endian=little '$od_endian_tmp'" \
        "$F_TOOL -An -t x4 --endian=little '$od_endian_tmp'"

    run_stdout_test "--endian=big -t x4" \
        "$GNU_TOOL -An -t x4 --endian=big '$od_endian_tmp'" \
        "$F_TOOL -An -t x4 --endian=big '$od_endian_tmp'"

    echo ""
    echo "=== GNU Upstream: Multiple Format Types ==="

    # From od-multiple-t.sh: multiple -t specifiers align properly
    printf '%s\n' $(seq 19) > "$test_dir/seq19.txt"
    run_stdout_test "-t x1 -t c dual format" \
        "$GNU_TOOL -t x1 -t c '$test_dir/seq19.txt'" \
        "$F_TOOL -t x1 -t c '$test_dir/seq19.txt'"

    run_stdout_test "-t x2 -t d2 dual format" \
        "$GNU_TOOL -t x2 -t d2 '$test_dir/seq19.txt'" \
        "$F_TOOL -t x2 -t d2 '$test_dir/seq19.txt'"

    echo ""
    echo "=== GNU Upstream: 8-Byte Hex ==="

    # From od-x8.sh: od -t x8 on 8 byte input
    printf 'abcdefgh' > "$test_dir/x8_input.bin"
    run_stdout_test "-t x8 eight-byte hex" \
        "$GNU_TOOL -An -t x8 '$test_dir/x8_input.bin'" \
        "$F_TOOL -An -t x8 '$test_dir/x8_input.bin'"

    echo ""
    echo "=== GNU Upstream: Float Output ==="

    # From od-float.sh: float format on binary data
    run_stdout_test "-t fF float format" \
        "printf '\\xfe\\xfe\\xfe\\xfe\\xff\\xfe\\xfe\\xfe' | $GNU_TOOL -t fF" \
        "printf '\\xfe\\xfe\\xfe\\xfe\\xff\\xfe\\xfe\\xfe' | $F_TOOL -t fF"

    run_stdout_test "-t fD double format" \
        "printf '\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xff\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe' | $GNU_TOOL -t fD" \
        "printf '\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xff\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe\\xfe' | $F_TOOL -t fD"
