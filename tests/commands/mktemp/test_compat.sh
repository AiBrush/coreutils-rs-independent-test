...
GNU_TOOL=$(resolve_gnu_tool "mktemp")
F_TOOL="fmktemp"
...
run_mktemp_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Bad Unicode Handling ==="

    # GNU bad-unicode.sh: test mktemp with bad unicode in suffix and tmpdir
    # Skipped: requires bad_unicode helper function and specific non-default locales
    skip_test "bad unicode suffix" "Requires non-default locales (LOCALE_FR, LOCALE_FR_UTF8)"
    skip_test "bad unicode tmpdir" "Requires non-default locales (LOCALE_FR, LOCALE_FR_UTF8)"
