...
EOF
    ...
    echo ""
    echo "=== GNU Upstream: Large Split (1000 parts) ==="
    ...
    run_stdout_test "csplit 1000 lines by /./ {*}" \
        "(cd '$gnu_dir' && $GNU_TOOL '$test_dir/input1000' '/./' '{*}' 2>/dev/null | tail -5)" \
        "(cd '$f_dir' && $F_TOOL '$test_dir/input1000' '/./' '{*}' 2>/dev/null | tail -5)"

    echo ""
    echo "=== GNU Upstream: Basic Splitting ==="

    # From csplit.sh: csplit with empty line pattern and line number
    local csplit_in1=$(make_temp "$(printf 'a\n\n\n')")
    register_temp "$csplit_in1"
    run_stdout_test "csplit empty-line pattern + line 2" \
        "(cd '$test_dir' && $GNU_TOOL '$csplit_in1' '/^\$/' 2 2>&1)" \
        "(cd '$test_dir' && $F_TOOL '$csplit_in1' '/^\$/' 2 2>&1)"

    # From csplit.sh: csplit with negative offset pattern
    local csplit_in2=$(make_temp "$(printf '\na\n')")
    register_temp "$csplit_in2"
    run_stdout_test "csplit /a/-1 {*} negative offset" \
        "(cd '$test_dir' && rm -f xx* && $GNU_TOOL '$csplit_in2' '/a/-1' '{*}' 2>&1)" \
        "(cd '$test_dir' && rm -f xx* && $F_TOOL '$csplit_in2' '/a/-1' '{*}' 2>&1)"

    # From csplit.sh: csplit line 0 should fail
    local csplit_in3=$(make_temp "$(printf '\n')")
    register_temp "$csplit_in3"
    run_exit_code_test "csplit line 0 (error)" \
        "$GNU_TOOL '$csplit_in3' 0 2>&1" \
        "$F_TOOL '$csplit_in3' 0 2>&1"

    # From csplit.sh: csplit with decreasing line numbers should fail
    run_exit_code_test "csplit decreasing line numbers 2 1" \
        "$GNU_TOOL '$csplit_in3' 2 1 2>&1" \
        "$F_TOOL '$csplit_in3' 2 1 2>&1"

    # From csplit.sh: csplit on empty input
    run_exit_code_test "csplit /dev/null line 1 (error)" \
