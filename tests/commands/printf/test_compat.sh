...
run_printf_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Hex Escape Sequences ==="

    # From printf-hex.sh: only two hex digits consumed in \xHH
    run_stdout_test "\\x7e3 (only 2 hex digits)" \
        "$GNU_TOOL '\\x7e3\\n'" \
        "$F_TOOL '\\x7e3\\n'"

    echo ""
    echo "=== GNU Upstream: %q Quoting ==="

    # From printf-quote.sh: %q shell quoting for various inputs
    run_stdout_test "%q empty string" \
        "$GNU_TOOL '%q\\n' ''" \
        "$F_TOOL '%q\\n' ''"

    run_stdout_test "%q single quote" \
        "$GNU_TOOL '%q\\n' \"'\"" \
        "$F_TOOL '%q\\n' \"'\""

    run_stdout_test "%q simple word" \
        "$GNU_TOOL '%q\\n' a" \
        "$F_TOOL '%q\\n' a"

    run_stdout_test "%q with space" \
        "$GNU_TOOL '%q\\n' 'a b'" \
        "$F_TOOL '%q\\n' 'a b'"

    run_stdout_test "%q tilde prefix" \
        "$GNU_TOOL '%q\\n' '~a'" \
        "$F_TOOL '%q\\n' '~a'"

    run_stdout_test "%q tilde suffix (no quote)" \
        "$GNU_TOOL '%q\\n' 'a~'" \
        "$F_TOOL '%q\\n' 'a~'"

    echo ""
    echo "=== GNU Upstream: Basic printf Scenarios ==="

    # From printf.sh: three methods of specifying ESC character
    run_stdout_test "\\x1b \\33 \\e escape methods" \
        "$GNU_TOOL '\\x1b\\n\\33\\n\\e\\n' | od -An -tx1" \
        "$F_TOOL '\\x1b\\n\\33\\n\\e\\n' | od -An -tx1"

    # From printf.sh: printf -- 'foo\n' (double dash)
    run_stdout_test "-- double dash prefix" \
        "$GNU_TOOL -- 'foo\\n'" \
        "$F_TOOL -- 'foo\\n'"

    # From printf.sh: %*s dynamic width
    run_stdout_test "%*s dynamic width -3" \
        "$GNU_TOOL '1 %*sy\\n' -3 x" \
        "$F_TOOL '1 %*sy\\n' -3 x"

    # From printf.sh: \x40 hex escape in format
    run_stdout_test "\\x40 hex in format" \
        "$GNU_TOOL '\\x40\\n'" \
        "$F_TOOL '\\x40\\n'"

    # From printf.sh: %+d with sign
    run_stdout_test "%+d positive sign" \
        "$GNU_TOOL '% +d\\n' 234" \
        "$F_TOOL '% +d\\n' 234"

    # From printf.sh: %*d dynamic width negative
    run_stdout_test "%*d negative width" \
        "$GNU_TOOL '%*dx\\n' -2 0" \
        "$F_TOOL '%*dx\\n' -2 0"

    # From printf.sh: %*c dynamic width char
    run_stdout_test "%*c dynamic width" \
        "$GNU_TOOL '%*c\\n' 2 x" \
        "$F_TOOL '%*c\\n' 2 x"

    # From printf.sh: character value with double quote
    run_stdout_test "%d double-quote char value" \
        "$GNU_TOOL '%d\\n' '\"a'" \
        "$F_TOOL '%d\\n' '\"a'"
