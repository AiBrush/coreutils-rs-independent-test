...
GNU_TOOL=$(resolve_gnu_tool "mknod")
F_TOOL="fmknod"
...
run_mknod_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: FIFO Mode with Restrictive Umask ==="

    # From GNU mknod.sh: mknod -m with numeric mode under umask 777
    run_test "FIFO mode 734 under umask 777" \
        "(umask 777 && $GNU_TOOL -m 734 '$WORK_DIR/gnu_f1' p && stat -c '%a' '$WORK_DIR/gnu_f1')" \
        "(umask 777 && $F_TOOL -m 734 '$WORK_DIR/f_f1' p && stat -c '%a' '$WORK_DIR/f_f1')"

    # From GNU mknod.sh: mknod --mode with symbolic mode under umask 777
    run_test "FIFO symbolic mode ug+rw,o+r under umask 777" \
        "(umask 777 && $GNU_TOOL --mode='ug+rw,o+r' '$WORK_DIR/gnu_f4' p && stat -c '%a' '$WORK_DIR/gnu_f4')" \
        "(umask 777 && $F_TOOL --mode='ug+rw,o+r' '$WORK_DIR/f_f4' p && stat -c '%a' '$WORK_DIR/f_f4')"
