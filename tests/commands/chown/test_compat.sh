...
GNU_TOOL=$(resolve_gnu_tool "chown")
F_TOOL="fchown"
...
run_chown_tests() {
    ...
    # === GNU Upstream: Dereference Dangling Symlink ===
    echo ""
    echo "=== GNU Upstream: Dereference Dangling Symlink ==="

    # From deref.sh: chown --dereference on a dangling symlink should fail
    ln -sf "no-such-target-$$" "$test_dir/dangle" 2>/dev/null || true
    run_exit_code_test "deref dangling symlink" \
        "$GNU_TOOL --dereference $current_user '$test_dir/dangle' 2>&1" \
        "$F_TOOL --dereference $current_user '$test_dir/dangle' 2>&1"

    # === GNU Upstream: Preserve Root ===
    echo ""
    echo "=== GNU Upstream: Preserve Root ==="

    # From preserve-root.sh: chown -R --preserve-root on / should fail (non-root)
    if [[ "$(id -u)" -ne 0 ]]; then
        run_exit_code_test "-R --preserve-root on /" \
            "$GNU_TOOL -R --preserve-root 0 / 2>&1" \
            "$F_TOOL -R --preserve-root 0 / 2>&1"
    fi

    # === GNU Upstream: Separator Parsing ===
    echo ""
    echo "=== GNU Upstream: Separator Parsing ==="

    # From separator.sh: chown with empty string should succeed
    echo "sep_test" > "$test_dir/gnu_sep"
    echo "sep_test" > "$test_dir/f_sep"
    run_exit_code_test "chown empty string (no-op)" \
        "$GNU_TOOL '' '$test_dir/gnu_sep'" \
        "$F_TOOL '' '$test_dir/f_sep'"

    # chown user:group using colon separator
    run_exit_code_test "chown user:group colon sep" \
        "$GNU_TOOL '$current_user:$current_group' '$test_dir/gnu_sep'" \
        "$F_TOOL '$current_user:$current_group' '$test_dir/f_sep'"

    # chown :group (change group only)
    echo "grp_test" > "$test_dir/gnu_grp"
    echo "grp_test" > "$test_dir/f_grp"
    run_exit_code_test "chown :group (group only)" \
        "$GNU_TOOL ':$current_group' '$test_dir/gnu_grp'" \
        "$F_TOOL ':$current_group' '$test_dir/f_grp'"

    # chown user: (user and default group)
    echo "usrd_test" > "$test_dir/gnu_usrd"
    echo "usrd_test" > "$test_dir/f_usrd"
    run_exit_code_test "chown user: (user + default group)" \
        "$GNU_TOOL '$current_user:' '$test_dir/gnu_usrd'" \
        "$F_TOOL '$current_user:' '$test_dir/f_usrd'"

    # === GNU Upstream: Separator Dot Notation ===
    echo ""
    echo "=== GNU Upstream: Separator Dot Notation ==="

    # From separator.sh: chown supports '.' as separator when no ambiguity
    # Only use dot separator when neither user nor group contain dots
    local current_uid current_gid
    current_uid=$(id -u)
    current_gid=$(id -g)

    # Test with numeric UID.GID (dot separator)
    echo "dot_test" > "$test_dir/gnu_dot"
    echo "dot_test" > "$test_dir/f_dot"
    run_exit_code_test "GNU: chown uid.gid (dot separator)" \
        "$GNU_TOOL '$current_uid.$current_gid' '$test_dir/gnu_dot'" \
        "$F_TOOL '$current_uid.$current_gid' '$test_dir/f_dot'"

    # Test with just :group (colon, numeric)
    echo "colongrp" > "$test_dir/gnu_colongrp"
    echo "colongrp" > "$test_dir/f_colongrp"
    run_exit_code_test "GNU: chown :gid (numeric group only)" \
        "$GNU_TOOL ':$current_gid' '$test_dir/gnu_colongrp'" \
        "$F_TOOL ':$current_gid' '$test_dir/f_colongrp'"

    # === GNU Upstream: Basic --from (Requires Root) ===
    echo ""
    echo "=== GNU Upstream: Basic --from (Requires Root) ==="

    # From basic.sh: chown --from requires root for meaningful ownership changes
    skip_test "GNU: chown --from=owner:group filter" "Requires root"
