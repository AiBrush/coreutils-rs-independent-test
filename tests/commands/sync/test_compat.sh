...
GNU_TOOL=$(resolve_gnu_tool "sync")
F_TOOL="fsync"
...
run_sync_tests() {
    ...
    # === GNU Upstream: Conflicting Options ===
    echo ""
    echo "=== GNU Upstream: Conflicting Options ==="

    run_exit_code_test "--data --file-system conflict" \
        "$GNU_TOOL --data --file-system 2>&1" \
        "$F_TOOL --data --file-system 2>&1"

    run_exit_code_test "-d without operand" \
        "$GNU_TOOL -d 2>&1" \
        "$F_TOOL -d 2>&1"

    # === GNU Upstream: Write-Only File Sync ===
    echo ""
    echo "=== GNU Upstream: Write-Only File Sync ==="

    local WO_FILE="$WORK_DIR/writeonly.txt"
    touch "$WO_FILE"
    chmod 0200 "$WO_FILE"

    run_test "sync write-only file" \
        "$GNU_TOOL '$WO_FILE' && echo 'OK'" \
        "$F_TOOL '$WO_FILE' && echo 'OK'"

    # === GNU Upstream: Multiple Args with Errors ===
    echo ""
    echo "=== GNU Upstream: Multiple Args with Errors ==="

    run_exit_code_test "mixed existing and nonexistent files" \
        "$GNU_TOOL '$WORK_DIR/syncfile.txt' /tmp/nofile_$$ 2>&1" \
        "$F_TOOL '$WORK_DIR/syncfile.txt' /tmp/nofile_$$ 2>&1"
