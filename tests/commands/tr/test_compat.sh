...
run_tr_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Case Class Translation with Extension ==="

    # GNU tr-case-class.sh: [:lower:] translated to 0-9 extends last char
    run_test "lower class to digit range extends" \
        "echo 'abcdefghijklmnopqrstuvwxyz' | $GNU_TOOL '[:lower:]' '0-9'" \
        "echo 'abcdefghijklmnopqrstuvwxyz' | $F_TOOL '[:lower:]' '0-9'"

    # GNU tr-case-class.sh: double lower mapped to upper+digits
    run_test "double lower class to upper+digits" \
        "echo 'abcdefghijklmnopqrstuvwxyz' | $GNU_TOOL '[:lower:][:lower:]' '[:upper:]0-9'" \
        "echo 'abcdefghijklmnopqrstuvwxyz' | $F_TOOL '[:lower:][:lower:]' '[:upper:]0-9'"

    echo ""
    echo "=== GNU Upstream: Case Class Alignment Validation ==="

    # GNU tr-case-class.sh: misaligned case classes should fail
    run_exit_code_test "misaligned A-Z[:lower:] vs a-y[:upper:]" \
        "echo '' | $GNU_TOOL 'A-Z[:lower:]' 'a-y[:upper:]' 2>&1" \
        "echo '' | $F_TOOL 'A-Z[:lower:]' 'a-y[:upper:]' 2>&1"

    run_exit_code_test "misaligned [:upper:][:lower:] vs a-y[:upper:]" \
        "echo '' | $GNU_TOOL '[:upper:][:lower:]' 'a-y[:upper:]' 2>&1" \
        "echo '' | $F_TOOL '[:upper:][:lower:]' 'a-y[:upper:]' 2>&1"

    run_exit_code_test "misaligned A-Y[:lower:] vs a-z[:upper:]" \
        "echo '' | $GNU_TOOL 'A-Y[:lower:]' 'a-z[:upper:]' 2>&1" \
        "echo '' | $F_TOOL 'A-Y[:lower:]' 'a-z[:upper:]' 2>&1"

    run_exit_code_test "misaligned A-Z[:lower:] vs [:lower:][:upper:]" \
        "echo '' | $GNU_TOOL 'A-Z[:lower:]' '[:lower:][:upper:]' 2>&1" \
        "echo '' | $F_TOOL 'A-Z[:lower:]' '[:lower:][:upper:]' 2>&1"

    run_exit_code_test "misaligned A-Z[:lower:] vs [:lower:]A-Z" \
        "echo '' | $GNU_TOOL 'A-Z[:lower:]' '[:lower:]A-Z' 2>&1" \
        "echo '' | $F_TOOL 'A-Z[:lower:]' '[:lower:]A-Z' 2>&1"

    # These should succeed (properly aligned)
    run_exit_code_test "aligned [:upper:][:lower:] vs a-z[:upper:]" \
        "echo '' | $GNU_TOOL '[:upper:][:lower:]' 'a-z[:upper:]' 2>&1" \
        "echo '' | $F_TOOL '[:upper:][:lower:]' 'a-z[:upper:]' 2>&1"

    run_exit_code_test "aligned [:upper:][:lower:] vs [:upper:]a-z" \
        "echo '' | $GNU_TOOL '[:upper:][:lower:]' '[:upper:]a-z' 2>&1" \
        "echo '' | $F_TOOL '[:upper:][:lower:]' '[:upper:]a-z' 2>&1"

    echo ""
    echo "=== GNU Upstream: Unmatched Case Class in SET1 ==="

    # GNU tr-case-class.sh: unmatched [:lower:] in SET1 translates matching chars
    run_test "unmatched [:lower:] maps to repeated char" \
        "echo '#\$%123abcABC' | $GNU_TOOL '[:lower:]' '[.*]'" \
        "echo '#\$%123abcABC' | $F_TOOL '[:lower:]' '[.*]'"

    run_test "unmatched [:upper:] maps to repeated char" \
        "echo '#\$%123abcABC' | $GNU_TOOL '[:upper:]' '[.*]'" \
        "echo '#\$%123abcABC' | $F_TOOL '[:upper:]' '[.*]'"

    echo ""
    echo "=== GNU Upstream: Case Conversion with Extra Chars ==="

    # GNU tr-case-class.sh: case conversion plus extra character mapping
    run_test "lower-to-upper with extra dot-to-x mapping" \
        "echo 'abc.' | $GNU_TOOL '[:lower:].' '[:upper:]x'" \
        "echo 'abc.' | $F_TOOL '[:lower:].' '[:upper:]x'"

    echo ""
    echo "=== GNU Upstream: String1 Longer Than String2 Error ==="

    # GNU tr-case-class.sh: trailing space makes SET1 longer, SET2 ends with class
    run_exit_code_test "SET1 longer with trailing space" \
        "echo '' | $GNU_TOOL '[:upper:] ' '[:lower:]' 2>&1" \
        "echo '' | $F_TOOL '[:upper:] ' '[:lower:]' 2>&1"
