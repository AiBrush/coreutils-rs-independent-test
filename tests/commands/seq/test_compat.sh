...
run_seq_tests() {
    ...
    echo ""
    echo "=== GNU Upstream: Output Precision ==="

    # From seq-precision.sh: integer output near boundaries
    run_test "seq 999999 1000000" \
        "$GNU_TOOL 999999 1000000" \
        "$F_TOOL 999999 1000000"

    # From seq-precision.sh: hex input
    run_test "seq hex 0xF423F 0xF4240" \
        "$GNU_TOOL 0xF423F 0xF4240" \
        "$F_TOOL 0xF423F 0xF4240"

    # From seq-precision.sh: small step precision
    run_test "seq 0 0.000001 0.000003" \
        "$GNU_TOOL 0 0.000001 0.000003" \
        "$F_TOOL 0 0.000001 0.000003"

    # From seq-precision.sh: consistent precision for hex float step
    run_test "seq 1 0x1p-1 2 (hex float)" \
        "$GNU_TOOL 1 0x1p-1 2" \
        "$F_TOOL 1 0x1p-1 2"

    # From seq-precision.sh: exponent notation handling
    run_test "seq 1.1e1 12 (scientific)" \
        "$GNU_TOOL 1.1e1 12" \
        "$F_TOOL 1.1e1 12"

    run_test "seq 11 1.2e1 (scientific last)" \
        "$GNU_TOOL 11 1.2e1" \
        "$F_TOOL 11 1.2e1"

    # From seq-precision.sh: -w with exponent notation
    run_test "-w 1.10000e5 equal width" \
        "$GNU_TOOL -w 1.10000e5 1.10000e5" \
        "$F_TOOL -w 1.10000e5 1.10000e5"

    echo ""
    echo "=== GNU Upstream: Extra Number Logic ==="

    # From seq-extra-number.sh: floating point extra number at boundary
    run_test "seq 0 0.000001 0.000003 (extra num)" \
        "$GNU_TOOL 0 0.000001 0.000003" \
        "$F_TOOL 0 0.000001 0.000003"

    # From seq-extra-number.sh: single value with -f
    run_test "seq -f %g= 1000000 1000000" \
        "$GNU_TOOL -f '%g=' 1000000 1000000" \
        "$F_TOOL -f '%g=' 1000000 1000000"
