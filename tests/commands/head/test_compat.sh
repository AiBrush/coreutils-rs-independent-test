...
GNU_TOOL=$(resolve_gnu_tool "head")
F_TOOL="fhead"
...
run_head_tests() {
    ...
    # === GNU Upstream: head -c Behavior ===
    echo ""
    echo "=== GNU Upstream: head -c ==="

    # From head-c.sh: sequential head -c1 calls on same input
    local head_tmp
    head_tmp=$(make_temp "$(printf 'abc')")
    register_temp "$head_tmp"

    run_test "-c1 first byte" \
        "$GNU_TOOL -c1 '$head_tmp'" \
        "$F_TOOL -c1 '$head_tmp'"

    # head -c with negative value (all but last N bytes)
    local head_neg_tmp
    head_neg_tmp=$(make_temp "$(printf 'abc\ndef\n')")
    register_temp "$head_neg_tmp"

    run_test "-c -4 all but last 4 bytes" \
        "$GNU_TOOL -c -4 '$head_neg_tmp'" \
        "$F_TOOL -c -4 '$head_neg_tmp'"

    # === GNU Upstream: head File Position ===
    echo ""
    echo "=== GNU Upstream: head File Position ==="

    # From head-pos.sh: verify head leaves file position correctly
    local head_pos_tmp
    head_pos_tmp=$(make_temp "$(printf 'a\nb\n')")
    register_temp "$head_pos_tmp"

    run_test "-n 1 on two-line file" \
        "$GNU_TOOL -n 1 '$head_pos_tmp'" \
        "$F_TOOL -n 1 '$head_pos_tmp'"

    run_test "-n -1 on two-line file" \
        "$GNU_TOOL -n -1 '$head_pos_tmp'" \
        "$F_TOOL -n -1 '$head_pos_tmp'"

    # From head-pos.sh: large file with -n -N
    run_test "-n -50000 on large seq" \
        "seq 70000 | $GNU_TOOL -n -50000 | wc -l" \
        "seq 70000 | $F_TOOL -n -50000 | wc -l"

    # === GNU Upstream: head -c Sequential Reads ===
    echo ""
    echo "=== GNU Upstream: head -c Sequential Reads ==="

    # From head-c.sh: sequential head -c1 calls consume successive bytes
    # Verify that 'head -c1' on "abc" gets 'a'
    local head_seq_tmp
    head_seq_tmp=$(make_temp "$(printf 'abc')")
    register_temp "$head_seq_tmp"

    run_test "-c1 on 3-byte file (first byte only)" \
        "$GNU_TOOL -c1 '$head_seq_tmp'" \
        "$F_TOOL -c1 '$head_seq_tmp'"

    run_test "-c2 on 3-byte file" \
        "$GNU_TOOL -c2 '$head_seq_tmp'" \
        "$F_TOOL -c2 '$head_seq_tmp'"

    run_test "-c3 on 3-byte file (full content)" \
        "$GNU_TOOL -c3 '$head_seq_tmp'" \
        "$F_TOOL -c3 '$head_seq_tmp'"

    run_test "-c4 on 3-byte file (beyond end)" \
        "$GNU_TOOL -c4 '$head_seq_tmp'" \
        "$F_TOOL -c4 '$head_seq_tmp'"

    # From head-c.sh: head -c -4 on "abc\ndef\n" leaves "bc"
    local head_c_neg_tmp
    head_c_neg_tmp=$(make_temp "$(printf 'abc\ndef\n')")
    register_temp "$head_c_neg_tmp"

    run_test "-c -4 leaves first portion of file" \
        "$GNU_TOOL -c -4 '$head_c_neg_tmp'" \
        "$F_TOOL -c -4 '$head_c_neg_tmp'"

    run_test "-c -1 on multiline file" \
        "$GNU_TOOL -c -1 '$head_c_neg_tmp'" \
        "$F_TOOL -c -1 '$head_c_neg_tmp'"

    run_test "-c -7 on 8-byte file" \
        "$GNU_TOOL -c -7 '$head_c_neg_tmp'" \
        "$F_TOOL -c -7 '$head_c_neg_tmp'"

    run_test "-c -8 on 8-byte file (empty result)" \
        "$GNU_TOOL -c -8 '$head_c_neg_tmp'" \
        "$F_TOOL -c -8 '$head_c_neg_tmp'"

    # head-write-error.sh: skipped (requires /dev/full and getlimits_)
    skip_test "head write error to /dev/full" "Requires /dev/full (Linux-specific) and getlimits_"

    # head-c.sh: /proc and /sys tests skipped (Linux-specific)
    skip_test "head -c on /proc/version" "Requires /proc filesystem (Linux-specific)"

    # head-c.sh: ulimit memory allocation tests skipped
    skip_test "head -c memory allocation limits" "Requires ulimit and getlimits_"
