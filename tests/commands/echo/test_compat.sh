...
run_echo_tests() {
    ...
    # === GNU Upstream: Escape Specification Methods ===
    echo ""
    echo "=== GNU Upstream: Escape Specification Methods ==="

    run_stdout_test "hex escape \\x1b" \
        "$GNU_TOOL -n -e '\\x1b'" \
        "$F_TOOL -n -e '\\x1b'"

    run_stdout_test "octal escape \\33" \
        "$GNU_TOOL -n -e '\\33'" \
        "$F_TOOL -n -e '\\33'"

    run_stdout_test "octal escape \\033" \
        "$GNU_TOOL -n -e '\\033'" \
        "$F_TOOL -n -e '\\033'"

    run_stdout_test "four-digit octal \\0033" \
        "$GNU_TOOL -n -e '\\0033'" \
        "$F_TOOL -n -e '\\0033'"

    run_stdout_test "incomplete hex \\x literal" \
        "$GNU_TOOL -n -e '\\x\\n'" \
        "$F_TOOL -n -e '\\x\\n'"

    # === GNU Upstream: Double Dash Handling ===
    echo ""
    echo "=== GNU Upstream: Double Dash Handling ==="

    run_stdout_test "-- is printed with args" \
        "$GNU_TOOL -- 'foo'" \
        "$F_TOOL -- 'foo'"

    run_stdout_test "-n -e -- is printed with args" \
        "$GNU_TOOL -n -e -- 'foo\\n'" \
        "$F_TOOL -n -e -- 'foo\\n'"

    # === GNU Upstream: \\c Stops Processing ===
    echo ""
    echo "=== GNU Upstream: Backslash-c Stops Processing ==="

    run_stdout_test "-e \\c stops output mid-escape" \
        "$GNU_TOOL -e 'foo\\n\\cbar'" \
        "$F_TOOL -e 'foo\\n\\cbar'"

    # === GNU Upstream: Hex Value Sequences ===
    echo ""
    echo "=== GNU Upstream: Hex Value Sequences ==="

    run_stdout_test "hex values 4a-4f (JKLMNO)" \
        "$GNU_TOOL -n -e '\\x4a\\x4b\\x4c\\x4d\\x4e\\x4f'" \
        "$F_TOOL -n -e '\\x4a\\x4b\\x4c\\x4d\\x4e\\x4f'"

    run_stdout_test "hex values 4A-4F uppercase (JKLMNO)" \
        "$GNU_TOOL -n -e '\\x4A\\x4B\\x4C\\x4D\\x4E\\x4F'" \
        "$F_TOOL -n -e '\\x4A\\x4B\\x4C\\x4D\\x4E\\x4F'"

    run_stdout_test "literal backslash via -e" \
        "$GNU_TOOL -n -e '\\\\\\\\'" \
        "$F_TOOL -n -e '\\\\\\\\'"
    ...
    run_stdout_test "POSIXLY_CORRECT -n -E processes escapes" \
        "POSIXLY_CORRECT=1 $GNU_TOOL -n -E 'foo\\n'" \
        "POSIXLY_CORRECT=1 $F_TOOL -n -E 'foo\\n'"

    run_stdout_test "POSIXLY_CORRECT -nE treated as literal" \
        "POSIXLY_CORRECT=1 $GNU_TOOL -nE 'foo'" \
        "POSIXLY_CORRECT=1 $F_TOOL -nE 'foo'"

    run_stdout_test "POSIXLY_CORRECT -E -n treated as literal" \
        "POSIXLY_CORRECT=1 $GNU_TOOL -E -n 'foo'" \
        "POSIXLY_CORRECT=1 $F_TOOL -E -n 'foo'"

    run_stdout_test "POSIXLY_CORRECT --version treated as literal" \
        "POSIXLY_CORRECT=1 $GNU_TOOL --version" \
        "POSIXLY_CORRECT=1 $F_TOOL --version"
